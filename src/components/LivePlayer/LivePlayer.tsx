...
interface LivePlayerProps {
  /** URL m3u8 del stream */
  streamSrc: string;
  /** DPSDAIAssetKey para DAI */
  assetKey?: string | null;
  /** VAST preroll URL opcional */
  vastUrl?: string | null;
  /** Nombre de la señal */
  signalName?: string;
  /** Evento EPG actual */
  currentEvent?: EPGEvent | null;
  /** Si el player está en fullscreen */
  isFullscreen?: boolean;
  /** Callback para salir de fullscreen */
  onBack?: () => void;
}

// Config HLS específica de Live
...
export function LivePlayer({
  ...
  // Session params para xhrSetup (inyectados por useDaiStream)
  ...
  // xhrSetup para inyectar session params en segmentos .ts
  ...
  // ── DAI (hook específico Live) — debe ir antes de useHlsStream ──
  // Resuelve la URL del stream (vía DAI o fallback) y la expone como estado.
  ...
  // ── HLS (hook compartido) con config de Live ──
  // `src` viene de daiStream.resolvedStreamUrl: arranca cuando DAI resuelve la URL.
  ...
  // Cuando termina el preroll en fullscreen, mover foco al play/pause
  useEffect(() => {
  ...
  return (
    <FocusContext.Provider value={focusKey}>
      <div ref={(el) => {
        // Merge refs: Norigin focus + container
        (focusContainerRef as any).current = el;
        ...
        {/* Gradiente (solo en preview, no fullscreen) */}
        ...
        {/* Spinner de carga (no mostrar durante VAST preroll) */}
